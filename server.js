        ...
        if (baseUrl) {
            ...
            try {
                ...
                if (r.ok)
                    return res.json({ valid: true });
                if (r.status === 401 || r.status === 403)
                    return res.json({ valid: false, error: `HTTP ${r.status} — key rejected. Проверьте ключ или подождите — новые ключи могут активироваться не сразу.` });
                return res.json({ valid: true, warning: `HTTP ${r.status}` });
            }
            catch (e) {
                clearTimeout(timeout);
                if (e.name === "AbortError")
                    return res.json({ valid: false, error: "Таймаут соединения — проверьте сеть" });
                throw e;
            }
        }
